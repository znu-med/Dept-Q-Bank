...
const DarkMode = {
  ...
  init() {
    // Apply saved preference (or system preference via CSS @media)
    const saved = localStorage.getItem(this.STORAGE_KEY);
    if (saved) {
      document.documentElement.setAttribute('data-theme', saved);
    }
    // Re-bind toggle button whenever the DOM updates
    this._bindToggle();
    // Re-bind after every navigation (button is re-rendered each time)
    const observer = new MutationObserver(() => this._bindToggle());
    observer.observe(document.getElementById('app'), { childList: true, subtree: true });
  },

  _bindToggle() {
    const btn = document.getElementById('dark-toggle-btn');
    if (!btn || btn._dmBound) return;
    btn._dmBound = true;
  ...
  _updateIcon(btn) {
    ...
    btn.textContent = isDark ? this.ICON_LIGHT : this.ICON_DARK;
